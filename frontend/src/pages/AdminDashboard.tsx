...
export default function Dashboard() {
    ...
    const fetchItems = useCallback(async () => {
        try {
            ...
            toast({ title: "Error", description: "Failed to load admin items.", variant: "destructive" });
        } finally {
            setLoading(false);
        }
    }, [search, category, type, status, toast]);
    ...
    const approveItem = async (id: string) => {
        try {
            ...
            toast({ title: "Approved", description: "Item approved successfully.", variant: "default" });
            fetchItems();
        } catch {
    ...
    const deleteItem = async (id: string) => {
        try {
            ...
            toast({ title: "Deleted", description: "Item deleted successfully.", variant: "default" });
            fetchItems();
        } catch {
        ...
        <div className="min-h-screen bg-background">
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                    <div>
                        <h1 className="text-2xl font-bold text-foreground">Admin Review Board</h1>
                        <p className="text-muted-foreground text-sm mt-0.5">Approve or delete newly reported items before they appear to students.</p>
                    </div>
                    {user?.role !== "admin" && (
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                        ...
                        <p className="text-muted-foreground text-sm mt-1 mb-4">
                            {hasFilters ? "Try adjusting your filters." : "No reported items are available yet."}
                        </p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
